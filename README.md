...
- Side plaques: up to two independently configured tablets - a riveted steel plate (configurable thickness and rivet height) or an ornate parchment scroll (torn edges, rippling sheet, end rolls with knobs) - each with its own style, size, color, and text with the full font selection (name on the front, squad number on the back); placement snaps to one flat side on straight-edged bases and to the wide or narrow side of ovals; plaques and rim lettering are mutually exclusive, and plaque text needs a round base
